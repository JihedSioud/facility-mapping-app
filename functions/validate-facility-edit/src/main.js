...
export default async ({ req, res, log, error }) => {
  ...
  try {
    const { facilityId, facilityName, longitude, latitude, governorate, status } = JSON.parse(req.body);
    ...
    if (existing.documents.length > 0 && existing.documents[0].$id !== facilityId) {
      ...
      throw new Error(`Facility "${facilityName}" already exists in ${governorate}`);
    }
